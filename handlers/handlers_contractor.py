...
async def process_truck_category_contractor(callback: CallbackQuery, state: FSMContext):
    try:
...
async def process_truck_brand_contractor(message: Message, state: FSMContext):
    try:
...
async def process_truck_number_contractor(message: Message, state: FSMContext):
    try:
...
async def process_truck_comment_contractor(message: Message, state: FSMContext):
    try:
...
async def process_truck_visit_date_contractor(message: Message, state: FSMContext):
    try:
...
async def process_comment_and_save(message: Message, state: FSMContext):
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            contractor_id = contractor.id
            c_fio = contractor.fio or ""
            c_company = contractor.company or ""
            c_position = contractor.position or ""
        ...
        if status == "approved":
            ...
            staff_text = build_auto_approved_staff_notice(
                header_line=(
                    f"Пропуск от подрядчика {c_fio}, {c_company} — {c_position} одобрен автоматически"
                ),
            ...
            for tg_id in tg_ids:
                try:
                    await bot.send_message(
                        tg_id,
                        text=f'Поступила заявка на временный пропуск от подрядчика {c_fio}.\n(Пропуска > Временные пропуска > На утверждении)',
                        reply_markup=admin_reply_keyboard
                    )
                    await asyncio.sleep(0.05)
                except:
        ...
        await bot.send_message(
            1012882762,
            text=f'{count}_{c_fio}_{data.get("days")}_{new_visit_date.strftime("%d.%m.%Y")}',
        )
        await state.clear()
    except Exception as e:
        await bot.send_message(RAZRAB, f'{message.from_user.id} - {str(e)}')
